  ...
  const renderSalesSection = (sales) => {
    ...
    return (
      <div className="space-y-3 report-section">
        ...
        {products.length > 0 && (
          <div>
            <h6 className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mt-3 mb-2">Top Selling Products</h6>
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-slate-200 dark:border-slate-200 dark:border-white/[0.06]">
                  <th className="text-left py-1.5 font-bold text-slate-700 dark:text-slate-300">Product</th>
                  <th className="text-right py-1.5 font-bold text-slate-700 dark:text-slate-300">Revenue</th>
                  <th className="text-right py-1.5 font-bold text-slate-700 dark:text-slate-300">Qty</th>
                </tr>
              </thead>
              <tbody>
                {products.slice(0, 5).map((p, i) => (
                  <tr key={i} className="border-b border-slate-200 dark:border-slate-200 dark:border-white/[0.06]/40">
                    <td className="py-1.5 font-semibold text-slate-700 dark:text-slate-300">{p.product_name || `Product ${i+1}`}</td>
                    <td className="py-1.5 text-right text-slate-500 dark:text-slate-400">{formatCurrency(p.total_revenue)}</td>
                    <td className="py-1.5 text-right text-slate-500 dark:text-slate-400">{p.total_quantity || '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
  ...
  const renderInventorySection = (inv) => {
    ...
    return (
      <div className="space-y-3 report-section">
        ...
        {anomalies.length > 0 && (
          <div>
            <h6 className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mt-3 mb-2">Stock Anomalies</h6>
    ...
    return (
      <div className="space-y-3 report-section">
        ...
        {Object.keys(sentiments).length > 0 && (
          <div>
            <h6 className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mt-3 mb-2">Sentiment Distribution</h6>
              ...
              {Object.entries(sentiments).map(([key, val]) => {
                ...
                return (
                  <span key={key} className={`px-2.5 py-1 rounded-lg text-xs font-bold border ${
                    ...
                    'glass-surface text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-200 dark:border-white/[0.06]/40'
                  }`}>
        ...
        {complaints.length > 0 && (
          <div>
            <h6 className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mt-3 mb-2">Top Complaint Categories</h6>
            <ul className="space-y-1 text-xs">
              {complaints.slice(0, 5).map((c, i) => (
                <li key={i} className="text-slate-700 dark:text-slate-300 font-medium">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        {/* Tab Controls */}
        <div className="flex items-center gap-1.5 glass-surface border border-slate-200 dark:border-slate-200 dark:border-white/[0.06] p-1.5 rounded-xl self-start sm:self-center">
          <button
      ...
      {activeTab === 'generate' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {reportTemplates.map((template) => (
            <div
              key={template.id}
              className="glass-card backdrop-blur-md border border-slate-200 dark:border-slate-200 dark:border-white/[0.06] hover:-translate-y-1 hover:shadow-xl rounded-2xl p-6 shadow-sm flex flex-col justify-between transition-all duration-300 group"
            >
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  <div className="p-3 glass-surface/65 rounded-xl border border-slate-200 dark:border-slate-200 dark:border-white/[0.06] group-hover:scale-105 transition-transform duration-300">
                    {template.icon}
                  </div>
        ...
        <div className="glass-card backdrop-blur-md border border-slate-200 dark:border-slate-200 dark:border-white/[0.06] rounded-2xl shadow-md overflow-hidden">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <h4 className="text-lg font-bold text-slate-700 dark:text-slate-300">No reports generated yet</h4>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-slate-200 dark:border-slate-200 dark:border-white/[0.06] glass-surface glass-card/50">
                    <th className="px-6 py-4 text-xs font-bold text-slate-700 dark:text-slate-200 uppercase tracking-wider">Report Type</th>
                    <th className="px-6 py-4 text-xs font-bold text-slate-700 dark:text-slate-200 uppercase tracking-wider">Generated Date</th>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
          <div className="w-full max-w-md bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-3xl p-6 shadow-2xl space-y-4 animate-scale-up">
            ...
            <form onSubmit={handleGenerate} className="space-y-4">
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Export Format</label>
                <select
      ...
      {reportViewer && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in ">
          <div className="w-full max-w-3xl bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-3xl p-6 shadow-2xl space-y-4 animate-scale-up max-h-[90vh] overflow-y-auto ">
            ...
            {/* Document Content */}
            <div id="report-print-area" className="space-y-6 text-sm text-slate-700 dark:text-slate-300 p-6 bg-slate-50 dark:bg-navy-900/40 rounded-2xl border border-slate-200 dark:border-white/[0.06]">
              {/* Report Header */}
                ...
                <div className="space-y-6">
                  ...
                  {reportViewer.report_type === 'executive' && (
                    <>
                      ...
                      <hr className="border-slate-200 dark:border-slate-200 dark:border-white/[0.06]" />
              ...
              {/* Sign-off */}
              <div className="pt-6 border-t border-slate-200 dark:border-slate-200 dark:border-white/[0.06] flex justify-between items-center text-[10px] text-slate-500">
