...
export default function PreviewStep({ tempFileId, sourceType, mapping, onPreviewComplete, onBack }) {
  ...
  if (loading) {
    return (
      <div className="max-w-4xl mx-auto bg-navy-800/60 bg-navy-800/60 rounded-xl shadow-sm border border-white/[0.06] p-12 text-center">
        ...
        <h3 className="text-lg font-medium text-white text-white">Analyzing Your Data...</h3>
        <p className="text-slate-500 text-slate-400 mt-2">Checking data types and formatting</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-5xl mx-auto bg-navy-800/60 bg-navy-800/60 rounded-xl shadow-sm border border-white/[0.06] p-8">
      <h2 className="text-2xl font-bold text-white text-white mb-2">Preview & Validate</h2>
      ...
      {hasCriticalErrors ? (
        ...
        <>
          <div className="grid grid-cols-3 gap-4 mb-8">
            <div className="bg-navy-900/40 bg-navy-700/60 p-4 rounded-lg border border-white/[0.04] border-white/[0.06] text-center">
              <p className="text-sm text-slate-500 text-slate-400 mb-1">Total Rows</p>
            ...
            <div className={`p-4 rounded-lg border text-center ${stats.invalid > 0 ? 'bg-orange-50 border-orange-100 dark:bg-orange-900/20 dark:border-orange-800/30' : 'bg-navy-900/40 bg-navy-700/60 border-white/[0.04] border-white/[0.06]'}`}>
              <p className={`text-sm mb-1 ${stats.invalid > 0 ? 'text-orange-600 dark:text-orange-400' : 'text-slate-500 text-slate-400'}`}>Invalid Rows (Will be skipped)</p>
              <p className={`text-2xl font-bold ${stats.invalid > 0 ? 'text-orange-700 dark:text-orange-300' : 'text-white text-white'}`}>{stats.invalid}</p>
            </div>
          </div>

          <h3 className="text-lg font-medium text-white text-white mb-3">Data Preview</h3>
          <div className="overflow-x-auto border border-white/[0.06] border-white/[0.06] rounded-lg mb-8">
            <table className="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
              <thead className="bg-navy-900/40 bg-navy-700/60">
                <tr>
                  {previewData.length > 0 && Object.keys(previewData[0]).map((key) => (
                    <th key={key} scope="col" className="px-6 py-3 text-left text-xs font-bold text-slate-300 text-slate-200 uppercase tracking-wider">
                      {key}
                    </th>
                  ))}
                </tr>
