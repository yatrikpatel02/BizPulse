    ...
    <div className="max-w-2xl mx-auto bg-white dark:bg-slate-900 rounded-xl shadow-sm border dark:border-slate-800 p-12 text-center">
      {status === 'processing' && (
        <>
          ...
          <h2 className="text-2xl font-bold text-gray-900 dark:text-slate-100 mb-2">Importing Your Data...</h2>
          <p className="text-gray-500 dark:text-slate-400">Please wait while we save these records to your database. This might take a few moments.</p>
        </>
      )}
      ...
      {status === 'success' && (
        <>
          ...
          <h2 className="text-2xl font-bold text-gray-900 dark:text-slate-100 mb-2">Import Successful!</h2>
      ...
      {status === 'error' && (
        <>
          ...
          <h2 className="text-2xl font-bold text-gray-900 dark:text-slate-100 mb-2">Import Failed</h2>
          <div className="p-4 bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-400 rounded-lg text-sm border border-red-200 dark:border-red-800/50 mb-6 mx-auto max-w-md">
            {error}
          </div>
          <button
            onClick={onReset}
