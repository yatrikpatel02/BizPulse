...
REVIEWS_BANK = {
    5: [
        ...
        "Outstanding quality. Zero complaints.",
        "Highly recommend to everyone. Brilliant!"
    ],
    4: [
        "Very satisfied with my purchase. Fast delivery too.",
        "Excellent product. Will buy again.",
        ...
        "Sturdy build quality and works perfectly.",
        "Works like a charm. Very pleased.",
        "Packaging was nice, product works well.",
        "Minor issues but overall a great buy."
    ],
    3: [
    ...
    2: [
        ...
        "Broke too soon. Very disappointed."
    ],
    1: [
        ...
        "Does not match the description at all.",
        "Would not recommend. Waste of money.",
        "Complete waste. Never buying again."
    ]
}

# Date range: 2023-01-01 to 2025-06-30
start_date = datetime(2023, 1, 1)
total_days = (datetime(2025, 6, 30) - start_date).days + 1

# ============================================================
# 1. GENERATE sales.csv - read existing + add ~4000 more rows
# ============================================================
print("Reading existing sales.csv...")
existing_sales = []
with open("sample_data/sales.csv", "r", encoding="utf-8") as f:
    reader = csv.DictReader(f)
    for row in reader:
        existing_sales.append(row)

print(f"Existing sales rows: {len(existing_sales)}")

# Generate 4000 extra rows with different random seed
random.seed(2025)
new_sales = []
for _ in range(4000):
...
print(f"Generated sample_data/sales.csv with {len(all_sales)} rows.")


# ============================================================
# 2. GENERATE inventory.csv - monthly snapshots for all dates
# ============================================================
# Covers 2023-01 to 2025-06 = 30 months
months = []
...
# ============================================================
# 3. GENERATE reviews.csv - proportionally scaled to sales
# ============================================================
# ~1 review per 4-5 sales rows → ~2000 reviews for 9250 sales
NUM_REVIEWS = 2100
...
with open("sample_data/reviews.csv", "w", newline="", encoding="utf-8") as f:
    writer = csv.DictWriter(f, fieldnames=["review_date", "product_name", "category", "source", "author_name", "rating", "review_text"])
    writer.writeheader()
    writer.writerows(review_rows)

# Also update customer_reviews.csv
with open("sample_data/customer_reviews.csv", "w", newline="", encoding="utf-8") as f:
    writer = csv.DictWriter(f, fieldnames=["review_date", "product_name", "category", "source", "author_name", "rating", "review_text"])
    writer.writeheader()
    writer.writerows(review_rows)
